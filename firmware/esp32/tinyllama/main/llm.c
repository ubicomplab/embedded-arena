...
void malloc_run_state(RunState *s, Config *p)
{
    ...
    s->key_cache = (v4sf *)tmp;
    if (posix_memalign(&tmp, 16, p->n_layers * p->seq_len * kv_dim * sizeof(v4sf)) != 0) tmp = NULL;
    s->value_cache = (v4sf *)tmp;
                     ...
                     int *fd, v4sf **data, size_t *file_size)
{
    ...
    *file_size = ftell(file); // get the file size, in bytes
    fseek(file, sizeof(Config), SEEK_SET); // move back to just after the config for reading
    ...
    void *data_tmp = NULL;
    if (posix_memalign(&data_tmp, 16, weights_size) != 0) data_tmp = NULL;
    *data = (v4sf *)data_tmp;
    if (*data == NULL)
    {
    ...
    // Read the entire weights into memory
...
void free_transformer(Transformer *t)
{
    ...
    if (t->data != NULL && t->data != (void*)-1) // handle MAP_FAILED or NULL just in case
    {
        free(t->data);
    }
    if (t->fd != -1)
...
static token_emit_cb g_emit_hook = NULL;
static int g_min_tokens = 0;

void llm_set_emit_hook(token_emit_cb hook)
{
    g_emit_hook = hook;
}

void llm_set_min_tokens(int n)
{
    g_min_tokens = n;
}
...
void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, char *prompt, int steps, generated_complete_cb cb_done)
{
    ...
    while (pos < steps)
    {
        ...
        // Suppress the early-break until we've reached the configured minimum.
        if (next == 1 && pos >= g_min_tokens)
        {
            break;
        }
